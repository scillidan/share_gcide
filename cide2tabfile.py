...
def convert_special_characters(text, filename, logger):
    ...
    conversions = {"ç": 0, "¹": 0, "other_non_ascii": 0}
    ...
    for old, new in replacements.items():
        if old in text:
            ...
            logger.info(
                f"Replaced {count} '{repr(old)}' with '{repr(new)}' in {filename}"
            )
    ...
    if conversions["ç"] > 0:
        logger.info(
            f"Converted {conversions['ç']} 'ç' characters to '<cced/' in {filename}"
        )
    if conversions["¹"] > 0:
        logger.info(
            f"Converted {conversions['¹']} '¹' characters to '<sup>1</sup>' in {filename}"
        )

    # Log other problematic characters
    if problematic_chars:
        logger.warning(
            f"Found {len(problematic_chars)} other non-ASCII characters in {filename}:"
        )
        for pos, char, code in problematic_chars[:10]:  # Log first 10 problematic chars
            logger.warning(
                f"  Position {pos}: Character '{char}' (Unicode: U+{code:04X})"
            )
        if len(problematic_chars) > 10:
            logger.warning(
                f"  ... and {len(problematic_chars) - 10} more non-ASCII characters"
            )
...
def read_file_with_fallback(filename, logger):
    ...
    logger.info(
        f"Detected encoding for {filename}: {detected_encoding} (confidence: {confidence:.2f})"
    )
    ...
    for i, encoding in enumerate(encodings_to_try):
        try:
            ...
            converted_content = convert_special_characters(
                content, os.path.basename(filename), logger
            )
            ...
            if i == len(encodings_to_try) - 1:  # Last attempt failed
                logger.error(
                    f"Failed to read {filename} with all attempted encodings: {encodings_to_try}"
                )
                raise e
            logger.warning(
                f"Failed to read {filename} with {encoding}: {e}. Trying next encoding..."
            )
        except Exception as e:
    ...
    raise UnicodeDecodeError(
        "All encoding attempts failed", b"", 0, 0, "No suitable encoding found"
    )
...
def merge_cide_files(input_dir, output_file):
    ...
    """
    log_file = os.path.splitext(os.path.basename(output_file))[0] + "_cide2tabfile.log"
    ...
    try:
            ...
            for i, filename in enumerate(file_list, 1):
                ...
                try:
                    ...
                    if used_encoding != "us-ascii":
                        ...
                        logger.info(
                            f"File {file_basename} required {used_encoding} encoding"
                        )
        ...
        logger.info(
            f"Total '¹' to '<sup>1</sup>' conversions: {stats['¹_conversions']}"
        )
...
if __name__ == "__main__":
    ...
    if success:
        print(f"\nMerge completed successfully! Check {log_file} for details.")
    else:
        print(f"\nMerge completed with errors. Check {log_file} for details.")
